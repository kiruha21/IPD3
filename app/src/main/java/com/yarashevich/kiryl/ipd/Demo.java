...
//        authorCardBuilder.titleColor(ContextCompat.getColor(c, R.color.colorAccent));
        ...
        otherCardBuilder.addItem(new MaterialAboutActionItem.Builder()
                ...
                .subTextHtml("<i>Это приложение для Android основано мной (Кирилл Ярошевич, либо в электронном мире мой ник: Hadenix). Так как во время учёбы приходилось смотреть расписание занятий на сайте в PDF, и постоянно искать какую-то информацию, было написано собственно это приложение, чтобы облегчить процесс. По началу приложение писал ради забавы и интереса для себя, позже начал видеть что студенты пользуются приложением, стал править ошибки и доводить его до ума. Немного позже добавил информацию которой часто пользовался в институте. В конечном итоге начал распостранять через Google Play для студентов ИПД, и править ошибки приложения дальше. Вот такая история данного приложения.</i>")
                .setIconGravity(MaterialAboutActionItem.GRAVITY_TOP)
                .build());
